...
i = 1
while exists:
    if os.path.exists(f"recording_2kHz_{i}.wav"):
        i += 1
    else:
        exists = False
save_A = os.path.join(save_path, f"recording_2kHz_{i}.wav")
...
x = 1
while exists:
    if os.path.exists(f"Waveform_2kHz_{x}.png"):
        x += 1
    else:
        exists = False
...
plt.xlabel('Time (s)')
plt.xlim(0, t_audio)
save_fileA = os.path.join(save_path, f"Waveform_2kHz_{x}.png")
...
plt.xlim(0, t_audio)
plt.colorbar()
save_fileB = os.path.join(save_path, f"Spectogram_2kHz_{x}.png")
